    ...
    return {
        init: function() {
            ...
            function rendergoods(sid, num) {
                $.ajax({
                    url: 'http://10.31.161.56/dashboard/nubiyaphone/php/alldata.php',
                    dataType: 'json'
                }).done(function(data) {
                    $.each(data, function(index, value) {
                        if (sid === value.sid) { //确定当前的数据。克隆隐藏的盒子，进行赋值
                            ...
                            $clonebox.find('.goods-pic img').attr('sid', value.sid); //获取编号
